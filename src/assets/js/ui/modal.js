const dim = {
  in(time) {
    ...
    if (time) {
      document.querySelector('#dim').style.cssText =
        'pointer-events:auto;opacity:1;transition:opacity ' + time + 's;';
    } else {
      document.querySelector('#dim').style.cssText =
        'pointer-events:auto;opacity:1;transition:opacity 0s;';
    }
  },
  out(time) {
    ...
    if (time) {
      document.querySelector('#dim').style.cssText =
        'pointer-events:none;opacity:0;transition:opacity ' + time + 's;';
    } else {
      document.querySelector('#dim').style.cssText =
